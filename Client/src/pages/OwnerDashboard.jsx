...
const ICONS = {
  ...
  venues:   'M3 9.5L12 3l9 6.5V20a1 1 0 01-1 1H4a1 1 0 01-1-1V9.5z M9 21V12h6v9',
  ...
  image:    'M21 15l-5-5L5 21M3 3h18v18H3z M8.5 10a1.5 1.5 0 100-3 1.5 1.5 0 000 3z',
...
const THEMES = {
  dark: {
    ...
    muted:      '#5a7a94',
    primary:    '#1e9e8c',
    primaryBg:  'rgba(30,158,140,0.12)',
    gold:       '#C8A45B',
    sidebar:    '#060d12',
    card:       '#0d1821',
  },
  light: {
    ...
    muted:      '#6a8fa8',
    primary:    '#1e7a6e',
    primaryBg:  'rgba(30,122,110,0.08)',
    gold:       '#b8892a',
    sidebar:    '#0d1821',
    card:       '#ffffff',
  },
...
const StatCard = ({ label, value, sub, icon, color, T, delay }) => (
  ...
  >
    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
      <span style={{ fontSize: 12, fontWeight: 600, color: T.muted, textTransform: 'uppercase', letterSpacing: '0.8px' }}>{label}</span>
      <div style={{ width: 34, height: 34, borderRadius: 10, background: color + '20', display: 'flex', alignItems: 'center', justifyContent: 'center', color }}>
...
const AddVenueModal = ({ onClose, onSave, T }) => {
  const [form, setForm] = useState({
    name: '', description: '', pricePerHour: '', capacity: '',
    city: '', pincode: '', address: '',
    amenities: '', venueType: 'hall',
  });
  ...
  const handleSubmit = async () => {
    if (!form.name || !form.pricePerHour || !form.city) {
      setError('Name, price, and city are required.'); return;
    }
    setSaving(true);
    setError('');
    try {
      ...
      setError(e?.response?.data?.message || 'Failed to create venue');
    } finally {
      setSaving(false);
    }
  };
  ...
  const labelStyle = { fontSize: 11, fontWeight: 700, color: T.muted, textTransform: 'uppercase', letterSpacing: '0.8px', display: 'block', marginBottom: 6 };
      ...
      >
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
          <div>
            ...
            <input style={inputStyle} value={form.name} onChange={e => set('name', e.target.value)} placeholder="e.g. The Grand Hall" />
          </div>
          <div>
            <label style={labelStyle}>Description</label>
            <textarea style={{ ...inputStyle, minHeight: 80, resize: 'vertical' }} value={form.description} onChange={e => set('description', e.target.value)} placeholder="Describe your venue..." />
          </div>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
            <div>
              <label style={labelStyle}>Price / Hour (₹) *</label>
              <input style={inputStyle} type="number" value={form.pricePerHour} onChange={e => set('pricePerHour', e.target.value)} placeholder="5000" />
            </div>
            <div>
              <label style={labelStyle}>Capacity</label>
              <input style={inputStyle} type="number" value={form.capacity} onChange={e => set('capacity', e.target.value)} placeholder="200" />
            </div>
          </div>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
            <div>
            ...
            <div>
              ...
              <input style={inputStyle} value={form.pincode} onChange={e => set('pincode', e.target.value)} placeholder="560001" />
            </div>
          </div>
          <div>
          ...
          <div>
            ...
            <select style={inputStyle} value={form.venueType} onChange={e => set('venueType', e.target.value)}>
          ...
          <div>
            ...
            <input style={inputStyle} value={form.amenities} onChange={e => set('amenities', e.target.value)} placeholder="Parking, AC, Catering, WiFi" />
          </div>
          <div>
            ...
            <div style={{
              ...
              <input type="file" multiple accept="image/*" style={{ display: 'none' }} id="venue-images"
                onChange={e => setImages(Array.from(e.target.files).slice(0, 5))} />
              <label htmlFor="venue-images" style={{ cursor: 'pointer', color: T.muted, fontSize: 13 }}>
        ...
        <div style={{ display: 'flex', gap: 12, marginTop: 24 }}>
          <button onClick={onClose} style={{
            ...
            color: T.muted, fontWeight: 600, cursor: 'pointer', fontFamily: 'inherit',
          }}>Cancel</button>
          <button onClick={handleSubmit} disabled={saving} style={{
            flex: 2, padding: '12px', borderRadius: 12,
            background: saving ? T.muted : T.primary,
...
const EditVenueModal = ({ venue, onClose, onSave, T }) => {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      await onSave(venue._id, payload);
      onClose();
  ...
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
      ...
      <motion.div initial={{ scale: 0.92 }} animate={{ scale: 1 }} exit={{ scale: 0.92 }}
        style={{ background: T.surface, borderRadius: 20, padding: 28, width: '100%', maxWidth: 480, border: `1px solid ${T.border}` }}
        onClick={e => e.stopPropagation()}>
        <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 20 }}>
          <h2 style={{ color: T.text, fontSize: 18, fontWeight: 800, margin: 0 }}>Edit Venue</h2>
          <button onClick={onClose} style={{ background: 'none', border: 'none', color: T.muted, cursor: 'pointer' }}><Icon d={ICONS.x} size={20} /></button>
        </div>
        <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
          {[['name','Venue Name'], ['description','Description'], ['pricePerHour','Price / Hour (₹)'], ['capacity','Capacity'], ['amenities','Amenities (comma-separated)']].map(([k, lbl]) => (
            <div key={k}>
              ...
              {k === 'description'
                ? <textarea style={{ ...inputStyle, minHeight: 70, resize: 'vertical' }} value={form[k]} onChange={e => set(k, e.target.value)} />
                : <input style={inputStyle} value={form[k]} onChange={e => set(k, e.target.value)} />}
            </div>
          ))}
        </div>
        <div style={{ display: 'flex', gap: 12, marginTop: 20 }}>
          <button onClick={onClose} style={{ flex: 1, padding: '11px', borderRadius: 12, border: `1px solid ${T.border}`, background: 'transparent', color: T.muted, fontWeight: 600, cursor: 'pointer', fontFamily: 'inherit' }}>Cancel</button>
          <button onClick={handleSave} disabled={saving} style={{ flex: 2, padding: '11px', borderRadius: 12, background: T.primary, border: 'none', color: 'white', fontWeight: 700, cursor: 'pointer', fontFamily: 'inherit' }}>
...
const OwnerDashboardScreen = () => {
  ...
  const [theme, setTheme] = useState(() => localStorage.getItem('owner-theme') || 'dark');
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      showNotice('Failed to load data', true);
    } finally {
      setLoading(false);
    }
  }, []);
  ...
  // ── Pending bookings sorted by date ───────────────────────────────────
  ...
  return (
    <div style={{ display: 'flex', minHeight: '100vh', background: T.bg, fontFamily: "'DM Sans', sans-serif", color: T.text }}>
      ...
      <aside style={{
        ...
        alignItems: 'center', paddingTop: 20, paddingBottom: 20, gap: 6,
      ...
      >
        {/* Logo */}
        <div style={{ width: 38, height: 38, borderRadius: 12, background: T.primary, display: 'flex', alignItems: 'center', justifyContent: 'center', marginBottom: 24, cursor: 'pointer' }}
          onClick={() => navigate('/')}>
          <span style={{ color: 'white', fontWeight: 900, fontSize: 14 }}>BY</span>
        </div>
      ...
      {/* ── Main content ─────────────────────────────────────────────── */}
      <main style={{ flex: 1, marginLeft: 0, paddingBottom: 80 }} className="md:ml-[68px]">
        {/* Top bar */}
          ...
          {tab === 'venues' && (
            <motion.button whileHover={{ scale: 1.04 }} whileTap={{ scale: 0.97 }}
              ...
              style={{
                ...
                borderRadius: 50, background: T.primary, border: 'none',
        ...
        <AnimatePresence>
          {(notice || error) && (
            <motion.div initial={{ opacity: 0, y: -12 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }}
              style={{
                margin: '16px 24px 0', padding: '12px 18px', borderRadius: 12,
                background: error ? '#fef2f2' : T.primaryBg,
                border: `1px solid ${error ? '#fecaca' : T.primary}`,
                color: error ? '#dc2626' : T.primary, fontSize: 14, fontWeight: 600,
              }}>
              {notice || error}
            </motion.div>
          )}
        ...
        <div style={{ padding: '20px 24px' }}>
          ...
          {tab === 'overview' && (
            <div>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: 16, marginBottom: 28 }}>
                ...
                <StatCard label="Pending Bids" value={stats.pendingBids} sub="Awaiting your action" icon="bookings" color="#f59e0b" T={T} delay={0.05} />
                <StatCard label="Total Revenue" value={formatINR(stats.totalRevenue)} sub="From paid bookings" icon="home" color="#10b981" T={T} delay={0.1} />
                <StatCard label="Total Bookings" value={bookings.length} sub={`${bookings.filter(b => b.status === 'paid').length} paid`} icon="calendar" color="#8b5cf6" T={T} delay={0.15} />
              </div>

              {/* Recent pending bids */}
              <div style={{ background: T.surface, borderRadius: 16, border: `1px solid ${T.border}`, overflow: 'hidden' }}>
                <div style={{ padding: '18px 20px', borderBottom: `1px solid ${T.border}`, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                  <h3 style={{ color: T.text, fontWeight: 700, fontSize: 15, margin: 0 }}>Pending Bids</h3>
                ...
                {loading ? (
                  ...
                  <div style={{ padding: 40, textAlign: 'center', color: T.muted, fontSize: 14 }}>No pending bids right now</div>
                ) : (
                  <div>
                    {pendingBookings.slice(0, 5).map((b, i) => (
                      <motion.div key={b._id} initial={{ opacity: 0, x: -12 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: i * 0.05 }}
                        style={{
                          padding: '14px 20px', borderBottom: i < pendingBookings.slice(0, 5).length - 1 ? `1px solid ${T.border}` : 'none',
                        ...
                        <div style={{ display: 'flex', gap: 8 }}>
                          <motion.button whileTap={{ scale: 0.95 }}
                            ...
                            style={{ padding: '7px 14px', borderRadius: 8, background: '#dcfce7', color: '#166534', border: 'none', fontWeight: 700, fontSize: 12, cursor: 'pointer' }}>
                            ✓ Approve
                          </motion.button>
                          <motion.button whileTap={{ scale: 0.95 }}
                            onClick={() => handleBookingAction(b._id, 'rejected')}
                            disabled={statusLoading[b._id]}
                            style={{ padding: '7px 14px', borderRadius: 8, background: '#fee2e2', color: '#991b1b', border: 'none', fontWeight: 700, fontSize: 12, cursor: 'pointer' }}>
                            ✕ Reject
                          </motion.button>
                        </div>
                      </motion.div>
          ...
          {tab === 'venues' && (
            <div>
              {loading ? (
                <div style={{ textAlign: 'center', padding: 60, color: T.muted }}>Loading venues...</div>
              ) : venues.length === 0 ? (
                <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }}
                  style={{ textAlign: 'center', padding: 60, background: T.surface, borderRadius: 20, border: `1px solid ${T.border}` }}>
                  <div style={{ fontSize: 48, marginBottom: 16 }}>🏛️</div>
                  <p style={{ color: T.text, fontWeight: 700, fontSize: 18, marginBottom: 8 }}>No venues yet</p>
                  ...
                  <button onClick={() => setShowAddVenue(true)}
                    style={{ padding: '12px 28px', borderRadius: 50, background: T.primary, border: 'none', color: 'white', fontWeight: 700, cursor: 'pointer', fontFamily: 'inherit' }}>
                    Add Venue
                  </button>
                </motion.div>
              ) : (
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(300px, 1fr))', gap: 20 }}>
                  {venues.map((venue, i) => (
                    <motion.div key={venue._id}
                      ...
                      style={{ background: T.card, borderRadius: 18, border: `1px solid ${T.border}`, overflow: 'hidden', boxShadow: '0 4px 20px rgba(0,0,0,0.1)' }}>
                      {/* Image */}
                      <div style={{ height: 160, background: T.surfaceAlt, position: 'relative', overflow: 'hidden' }}>
                        {venue.images?.[0] ? (
                        ...
                        <div style={{
                          ...
                          color: venue.isActive ? '#166534' : '#991b1b',
                        }}>
                      ...
                      <div style={{ padding: '16px 18px' }}>
                        ...
                        <p style={{ color: T.primary, fontWeight: 700, fontSize: 14, margin: '0 0 14px' }}>
                          {formatINR(venue.pricePerHour)}<span style={{ color: T.muted, fontWeight: 400 }}>/hr</span>
                        </p>

                        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8 }}>
                          <button onClick={() => setEditingVenue(venue)}
                            style={{ padding: '9px', borderRadius: 10, border: `1px solid ${T.border}`, background: 'transparent', color: T.text, fontWeight: 600, fontSize: 12, cursor: 'pointer', fontFamily: 'inherit' }}>
                            ✏️ Edit
                          </button>
                          <button onClick={() => navigate(`/owner/venues/${venue._id}`)}
                            style={{ padding: '9px', borderRadius: 10, border: `1px solid ${T.border}`, background: 'transparent', color: T.text, fontWeight: 600, fontSize: 12, cursor: 'pointer', fontFamily: 'inherit' }}>
                            📋 Bookings
                          </button>
                          <button onClick={() => handleToggleActive(venue)}
                            style={{
                              gridColumn: '1 / -1', padding: '9px', borderRadius: 10, border: 'none',
                              background: venue.isActive ? '#fee2e2' : '#dcfce7',
                              color: venue.isActive ? '#991b1b' : '#166534',
          ...
          {tab === 'bookings' && (
            <div style={{ background: T.surface, borderRadius: 16, border: `1px solid ${T.border}`, overflow: 'hidden' }}>
              {loading ? (
                <div style={{ padding: 60, textAlign: 'center', color: T.muted }}>Loading bookings...</div>
              ) : allBookings.length === 0 ? (
                <div style={{ padding: 60, textAlign: 'center', color: T.muted, fontSize: 14 }}>No bookings yet</div>
              ) : (
                    ...
                    <tbody>
                      {allBookings.map((b, i) => (
                        <motion.tr key={b._id}
                          ...
                          <td style={{ padding: '14px 16px', fontSize: 14, fontWeight: 600, color: T.text }}>{b.venue?.name || '—'}</td>
                          ...
                          <td style={{ padding: '14px 16px', fontSize: 13, color: T.muted, whiteSpace: 'nowrap' }}>{formatDateIN(b.eventDate)}</td>
                          <td style={{ padding: '14px 16px', fontSize: 14, fontWeight: 700, color: T.primary }}>{formatINR(b.bidAmount)}</td>
                          <td style={{ padding: '14px 16px' }}><Badge status={b.status} /></td>
                          <td style={{ padding: '14px 16px' }}>
                            {b.status === 'pending' && (
                              <div style={{ display: 'flex', gap: 6 }}>
                                <button onClick={() => handleBookingAction(b._id, 'approved')} disabled={statusLoading[b._id]}
                                  style={{ padding: '6px 12px', borderRadius: 8, background: '#dcfce7', color: '#166534', border: 'none', fontWeight: 700, fontSize: 11, cursor: 'pointer' }}>
                                  ✓
                                </button>
                                <button onClick={() => handleBookingAction(b._id, 'rejected')} disabled={statusLoading[b._id]}
                                  style={{ padding: '6px 12px', borderRadius: 8, background: '#fee2e2', color: '#991b1b', border: 'none', fontWeight: 700, fontSize: 11, cursor: 'pointer' }}>
                                  ✕
                                </button>
                              </div>
                            )}
                            {b.status !== 'pending' && <span style={{ fontSize: 12, color: T.muted }}>{timeAgo(b.updatedAt)}</span>}
                          </td>
                        </motion.tr>
                      ))}
                    </tbody>
              ...
              <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }}
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: 16, marginBottom: 24 }}>
                  ...
                  <div>
                    ...
                    <div style={{ fontSize: 12, color: T.primary, fontWeight: 600, marginTop: 2 }}>
                ...
                <div style={{ borderTop: `1px solid ${T.border}`, paddingTop: 20 }}>
                  <p style={{ fontSize: 13, color: T.muted, marginBottom: 12 }}>
                    Switch between booker and venue owner mode instantly.
                  </p>
                  <motion.button whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.97 }}
                    ...
                    style={{
                      ...
                      background: switchingRole ? T.muted : T.primaryBg,
      ...
      <nav style={{
        ...
        background: T.sidebar, borderRadius: 50, padding: '8px 12px',
        boxShadow: '0 8px 32px rgba(0,0,0,0.35)',
        border: `1px solid ${T.border}`, zIndex: 200,
      }}
      ...
      >
        {NAV.map(item => (
          <motion.button key={item.id}
            ...
            whileTap={{ scale: 0.9 }}
            animate={{ scale: tab === item.id ? 1.1 : 1 }}
      ...
      <AnimatePresence>
        {showAddVenue && (
          <AddVenueModal onClose={() => setShowAddVenue(false)} onSave={handleCreateVenue} T={T} />
        )}
        {editingVenue && (
          <EditVenueModal venue={editingVenue} onClose={() => setEditingVenue(null)} onSave={handleUpdateVenue} T={T} />
        )}
      </AnimatePresence>
    </div>
  );
